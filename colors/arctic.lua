...
function utils.blend(foreground, alpha, background)
	local bg = utils.hex2rgb(background)
	local fg = utils.hex2rgb(foreground)
...
function utils.hex2ansi(c, to_type)
	local rgb = utils.hex2rgb(c)
...
---@param to_type string? which ANSI color type will be output, 'fg' or 'bg'
function utils.hl2ansi(hl, from_type, to_type)
	from_type = from_type or 'fg'
	to_type = to_type or 'fg'
	local color = vim.api.nvim_get_hl(0, { name = hl, link = false })[from_type]
	return color and utils.hex2ansi(string.format('#%06x', color), to_type) or ''
end
...
function utils.lighten(hex, amount, fg)
	return utils.blend(hex, amount, fg or utils.fg)
end

function utils.darken(hex, amount, bg)
	return utils.blend(hex, amount, bg or utils.bg)
end
...
local groups = {
	...
	DiagnosticVirtualTextHint = {
		fg = utils.lighten(hint_virtualtext_bg, 0.3),
		bg = hint_virtualtext_bg,
		italic = true,
	},
	...
	WinSeparator = { fg = '#5a5a5a' }, -- VSCode uses color win_separator
