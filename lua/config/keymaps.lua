...
map('ia', 'ellip', '…')
map('v', '??', 'y:h <C-R>"<cr>"', { desc = 'Show vim help' })
map('v', '?/', 'y:/ <C-R>"<cr>"', { desc = 'Search across the buffer' })
...
map('n', 'gX', function()
	vim.ui.open(('https://www.reverso.net/text-translation#sl=eng&tl=spa&text=%s'):format(vim.fn.expand('<cword>')))
end, { desc = '󰊿 Open Current Word Into reverso.net' })

map('x', 'gX', function()
	local lines = vim.fn.getregion(vim.fn.getpos('.'), vim.fn.getpos('v'), { type = vim.fn.mode() })
	vim.ui.open(
		('https://www.reverso.net/text-translation#sl=eng&tl=spa&text=%s'):format(vim.trim(table.concat(lines, ' ')))
	)
	vim.api.nvim_input('<Esc>')
end, { desc = '󰊿 Open Current Selection Into reverso.net' })
...
map('n', 'gs', function()
	...
	vim.fn.prompt_setcallback(bufnr, function(text)
		vim.ui.open(('https://unduck.link?q=%s'):format(vim.trim(text)))
		close_window_and_buffer()
	end)

	nkl.key.bmap({ 'n', 'i' }, '<C-q>', function()
		pcall(close_window_and_buffer)
	end, { buffer = bufnr })

	nkl.key.bmap({ 'n' }, '<esc>', function()
		pcall(close_window_and_buffer)
	end, { buffer = bufnr })
